...
function save_clearing_list() {
    ...
    if (vh_id.length !== 0) {
        ...
        $.post("views/commenSettingView.php", {action: 'save_clr_entry', form_data: data_array}, function (reply) {
            alertifyMsgDisplay(reply, 2000, function () {
                //clear_sup_form();
                // load_clearing_table();
            });
...
// update_vehicle_other_data
    ...
    if (parseInt(vh_id) !== 0) {
        confirm("Vehicle Sale", "Are you sure filled information is correct?", "No", "Yes", function () {
            $.post("views/commenSettingView.php", {action: 'update_vehicl_other', vh_id: vh_id, sold_date: sold_date, vehicle_price: vehicle_price, cust_id: cust_id}, function (e) {
//            console.log(e);
                if (e.complete === undefined || e.length === 0 || e === null) {
